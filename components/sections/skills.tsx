  ...
  return (
    <SectionWrapper id="skills">
            ...
            <button
              ...
              className={cn(
                ...
                activeCategory === index
                  ...
                  : "border border-white/[0.06] text-muted-foreground hover:border-white/10 hover:text-foreground"
              )}
            >
              <Icon size={14} />
              {category.category}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={activeCategory}
          className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02] sm:grid-cols-3 md:grid-cols-6"
        ...
        >
          {skills[activeCategory].items.map((skill, i) => (
            <motion.div
              key={skill}
              className="group flex items-center justify-center border border-white/[0.03] bg-background px-4 py-6 text-center transition-all duration-300 hover:bg-white/[0.03]"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
